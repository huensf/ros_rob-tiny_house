...
/* Begin of user declaration */
    ...
    case F_ar_d_1_id:
        ...
        if (ixF == F_av_g_1_id)
        {
            //printf("The slip angle is %f at time %f\n", angslip*180/M_PI, tsim);
            set_output(angslip, "a_gl_av_g_voit");
        }
        if (ixF == F_ar_g_1_id)
        {
            //printf("The slip angle is %f at time %f\n", angslip*180/M_PI, tsim);
            set_output(angslip, "a_gl_ar_g_voit");
        }
        ...
        else if (mbs_data->process == 2) // dynamic
        {
            ...
            /*if (ixF == F_av_g_1_id)
            {
                printf("Force verticale a la roue avant gauche au temps %f est %f\n", tsim, Fwhl_R[3]);
            }
            if (ixF == F_av_d_1_id)
            {
                printf("Force verticale a la roue avant droite au temps %f est %f\n", tsim, Fwhl_R[3]);
            }*/
        }
    ...
    case F_ar_d_rem_id:
		...
		tgc_car_kine_wheel(PxF, RxF, VxF, OMxF, mbs_data->user_model->FrontTire_rem.R, &pen, &rz, &angslip, &angcamb, &slip, Pcon, Vcon, Rt_ground, dxF);
		
        if (ixF == F_av_g_rem_id)
        {
            //printf("The slip angle is %f at time %f\n", angslip*180/M_PI, tsim);
            set_output(angslip, "a_gl_av_g_rem");
        }
        if (ixF == F_ar_g_rem_id)
        {
            //printf("The slip angle is %f at time %f\n", angslip*180/M_PI, tsim);
            set_output(angslip, "a_gl_ar_g_rem");
        }
        ...
        if (mbs_data->process == 1)// equil static
		{
			Fwhl_R[1] = 0.0;
			Fwhl_R[2] = 0.0;
			Fwhl_R[3] = mbs_data->user_model->FrontTire_rem.K * pen;
			Mwhl_R[1] = 0.0;
			Mwhl_R[2] = 0.0;
			Mwhl_R[3] = 0.0;
		}
