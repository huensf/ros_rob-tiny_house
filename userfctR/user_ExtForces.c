                       ...
                       MbsData *mbs_data, double tsim,int ixF)
{
	...
	double coeff_gliss_d = 1;
	double coeff_gliss_g = 1;
    double coeff_gliss_rem = 1;
	/*if(tsim > 5)
		coeff_gliss_g = 0.5;*/
    ...
    switch (ixF)
    {
        ...
        case F_ar_g_1_id:
			...
			else if (mbs_data->process == 2) // dynamic
			{
				if (pen > 0) {
	                ...
	                Fwhl_R[1] *= coeff_gliss_g;
				    Fwhl_R[2] *= coeff_gliss_g;
	                Mwhl_R[3] *= coeff_gliss_g;
				}
				else {
					Fwhl_R[1] = 0.0;
					Fwhl_R[2] = 0.0;
	    ...
	    case F_ar_d_1_id:
	        ...
	        else if (mbs_data->process == 2) // dynamic
	        {
				if (pen > 0) {
                    ...
                    Fwhl_R[1] *= coeff_gliss_d;
		            Fwhl_R[2] *= coeff_gliss_d;
	                Mwhl_R[3] *= coeff_gliss_d;
	            }
	            else {
	                Fwhl_R[1] = 0.0;
	                Fwhl_R[2] = 0.0;
        ...
        case F_ar_g_rem_id:
            ...
            mbs_car_kine_wheel(PxF, RxF, VxF, OMxF, mbs_data->user_model->Tire_rem.R, &pen, &rz, &angslip, &angcamb, &slip, Pcon, Vcon, Rt_ground, dxF);
            if (mbs_data->process == 1)// equil static 
            {
                Fwhl_R[1] = 0.0;
                Fwhl_R[2] = 0.0;
                Fwhl_R[3] = mbs_data->user_model->Tire_rem.K*pen;
                Mwhl_R[1] = 0.0;
                Mwhl_R[2] = 0.0;
                Mwhl_R[3] = 0.0;
            }
            else if (mbs_data->process == 2) // dynamic
            {
                if (pen > 0) {
                    ...
                    Fwhl_R[1] *= coeff_gliss_rem;
                    Fwhl_R[2] *= coeff_gliss_rem;
                    Mwhl_R[3] *= coeff_gliss_rem;
                }
                else {
                    Fwhl_R[1] = 0.0;
                    Fwhl_R[2] = 0.0;
    ...
    // {
    //     case F_av_g_1_id:
    //     set_output(Fwhl_I[3],"F_vert_av_g");
    //     break;

    //     case F_ar_g_1_id:
    //     set_output(Fwhl_I[3],"F_vert_ar_g");
    //     break;

    //     case F_av_d_1_id:
    //     set_output(Fwhl_I[3],"F_vert_av_d");
    //     break;

    //     case F_ar_d_1_id:
    //     set_output(Fwhl_I[3],"F_vert_ar_d");
    //     break;

    //     case F_av_g_rem_id:
    //     set_output(Fwhl_I[3],"F_vert_av_g_rem");
    //     break;

    //     case F_ar_g_rem_id:
    //     set_output(Fwhl_I[3],"F_vert_ar_g_rem");
    //     break;

    //     case F_av_d_rem_id:
    //     set_output(Fwhl_I[3],"F_vert_av_d_rem");
    //     break;

    //     case F_ar_d_rem_id:
    //     set_output(Fwhl_I[3],"F_vert_ar_d_rem");
    //     break;
    // }
